 ...
 */
var $ = require('jquery');
function addFile(path, parent, orig) {
  var resolved = addFile.resolve(path);

  // lookup failed
  if (null == resolved) {
    orig = orig || path;
    parent = parent || 'root';
    var err = new Error('Failed to addFile "' + orig + '" from "' + parent + '"');
    err.path = orig;
    err.parent = parent;
    err.addFile = true;
    throw err;
  ...
  var module = addFile.modules[resolved];

  // perform real addFile()
  ...
  if (!module._resolving && !module.exports) {
    ...
    module.call(this, mod.exports, addFile.relative(resolved), mod);
...
addFile.resolve = function(path) {
  ...
  for (var i = 0; i < paths.length; i++) {
    var path = paths[i];
    if (addFile.modules.hasOwnProperty(path)) return path;
    if (addFile.aliases.hasOwnProperty(path)) return addFile.aliases[path];
  }
};
...
addFile.normalize = function(curr, path) {
  var segs = [];
...
addFile.register = function(path, definition) {
  addFile.modules[path] = definition;
};
...
addFile.alias = function(from, to) {
  if (!addFile.modules.hasOwnProperty(from)) {
    throw new Error('Failed to alias "' + from + '", it does not exist');
  }
  addFile.aliases[to] = from;
};

/**
 * Return a addFile function relative to the `parent` path.
 *
 * @param {String} parent
 * @return {Function}
 * @api private
...
addFile.relative = function(parent) {
  var p = addFile.normalize(parent, '..');
  ...
  /**
   * The relative addFile() itself.
   */

  function localaddFile(path) {
    var resolved = localaddFile.resolve(path);
    return addFile(resolved, parent, path);
  }
  ...
  localaddFile.resolve = function(path) {
    var c = path.charAt(0);
    if ('/' == c) return path.slice(1);
    if ('.' == c) return addFile.normalize(p, path);
  ...
  localaddFile.exists = function(path) {
    return addFile.modules.hasOwnProperty(localaddFile.resolve(path));
  };

  return localaddFile;
};
addFile.register("component-emitter/index.js", function(exports, addFile, module){
...
addFile.register("side-comments/js/main.js", function(exports, addFile, module){
var _ = addFile('./vendor/lodash-custom.js');
var Section = addFile('./section.js');
var Emitter = addFile('emitter');
...
addFile.register("side-comments/js/section.js", function(exports, addFile, module){
var _ = addFile('./vendor/lodash-custom.js');
var Template = addFile('../templates/section.html');
var CommentTemplate = addFile('../templates/comment.html');
var mobileCheck = addFile('./helpers/mobile-check.js');
...
addFile.register("side-comments/js/vendor/lodash-custom.js", function(exports, addFile, module){
/**
 * @license
...
;(function() {
   ...
   * don't addFile escaping in HTML and have no special meaning unless they're part
  ...
  function template(text, data, options) {
    ...
    text.replace(reDelimiters, function(match, escapeValue, interpolateValue, esTemplateValue, evaluateValue, offset) {
      ...
      // the JS engine embedded in Adobe products addFiles returning the `match`
...
addFile.register("side-comments/js/helpers/mobile-check.js", function(exports, addFile, module){
...
addFile.alias("side-comments/js/main.js", "side-comments/index.js");
